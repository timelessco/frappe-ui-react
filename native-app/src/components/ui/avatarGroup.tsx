...
import { avatarTheme, Avatar, AvatarProps, AvatarSizes } from './avatar';

import { styleAdapter, cx, isUndefined, getValidChildren, createContext } from '@/lib/utils';
...
const AvatarGroupWrapper: React.FC<AvatarGroupWrapperProps> = ({ children, style, ...props }) => {
	return (
		<View {...props} className={cx(avatarTheme.group.base, styleAdapter(style))}>
			{children}
		</View>
	);
};

const [AvatarGroupProvider, useAvatarGroup] = createContext<AvatarGroupSharedProps>({
	strict: false,
...
export const AvatarGroup: React.FC<Partial<AvatarGroupProps>> = (props) => {
	 ...
	 */
	const childrenWithinMax = isUndefined(max) ? validChildren : validChildren.slice(0, max);
			...
			<AvatarGroupWrapper size={size} {...rest}>
				{childrenWithinMax.map((renderElement, index) => (
					<View
						...
						style={[
							showRing ? { borderColor: ringColor, backgroundColor: ringColor } : {},
							{
				...
				{excess > 0 ? (
					<View
						...
						style={[showRing ? { borderColor: ringColor, backgroundColor: ringColor } : {}]}>
