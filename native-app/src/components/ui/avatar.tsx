...
import { Image, ImageProps, ImageSourcePropType, Text, View, ViewProps } from 'react-native';
import Svg, { ClipPath, Defs, G, Path, Rect } from 'react-native-svg';

import { useAvatarGroup } from './avatarGroup';
import Icon from './icon';

import { cx, IconProps, isUndefined, passProps, styleAdapter } from '@/lib/utils';
...
export const DefaultUser: React.FC<IconProps> = ({ fill = '#525252', className = '' }) => {
	return (
		<Svg width="100%" height="100%" viewBox="0 0 12 12" fill="none" className={className}>
...
export const Avatar: React.FC<Partial<AvatarProps>> = forwardRef<View, Partial<AvatarProps>>(
	(props, ref) => {
		...
		const isSquared = isUndefined(avatarGroupProps) ? squared : avatarGroupProps.squared;
		...
		const avatarSize = isUndefined(avatarGroupProps)
			? (size as AvatarSizes)
			: avatarGroupProps.size;
		return (
			<View
				className={cx(
					avatarTheme.base,
				...
				{imageAvailable && src ? (
					...
					<Text
						className={cx(avatarTheme.initials.base, avatarTheme.initials.size[avatarSize])}
				...
				{status && (
					<AvatarStatus parentsBackground={parentsBackground} size={size} status={status} />
				)}
			</View>
		);
	}
...
export const AvatarStatus: React.FC<Pick<AvatarProps, 'status' | 'size' | 'parentsBackground'>> = ({
	...
	switch (status) {
		case 'active': {
			return (
				<View
					className={cx(avatarTheme.status.active.container, avatarTheme.status.position)}
					style={{ borderColor: parentsBackground }}>
					<View
						className={cx(avatarTheme.status.active.base, avatarTheme.status.active.size[size])}
					/>
				</View>
			);
		}
		case 'away': {
			return (
				<View
					className={cx(avatarTheme.status.away.container, avatarTheme.status.position)}
					style={{
		...
		case 'sleep': {
			return (
				<View
					className={cx(avatarTheme.status.sleep.container, avatarTheme.status.position)}
					style={[
						{
							borderColor: parentsBackground,
							backgroundColor: parentsBackground,
		...
		case 'pin': {
			return (
				<View
					className={cx(avatarTheme.status.pin.container, avatarTheme.status.position)}
					style={[
						{
							borderColor: parentsBackground,
							backgroundColor: parentsBackground,
